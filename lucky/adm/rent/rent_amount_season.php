...
if($com_no==""){
    ...
    $com_sql = "and rent_season_amount.rent_com_no='{$main_com_no[0]}'";
    $com_no = $main_com_no[0];

}else{
    $main_com_no = get_rentcar_company($com_no,"협력");
    $com_sql = "and rent_season_amount.rent_com_no='{$com_no}'";
}
if(!$sea_no){$sea_no=get_rentcar_season_no($com_no);}else{$sea_no=$sea_no;}

if($main_com_no[1]=="협력"){
    $company_sql = "and rent_season_amount.rent_car_no=rent_car_detail.rent_car_no";
}else{
    $company_sql = "and rent_season_amount.rent_car_no=rent_car_detail.no";
}
if($rent_car_type2!=""){
...
$sql = "select rent_season_list.no as list_no,
                ...
                rent_season_amount.rent_com_no,
                rent_season_amount.rent_car_no,
                rent_season_name,
                ...
                rent_season_week
         from rent_season_amount,rent_season_list,rent_car_detail where rent_season_list.no=rent_season_amount.rent_season_no {$company_sql} {$type_sql}  and  rent_season_no='{$sea_no}' $com_sql   order by rent_season_amount.no asc";
$rs  = $db->sql_query($sql);
if(!$rs) {
    echo "";
}else{
...
<div class="rent_season">
    ...
    <div >
        ...
        if(is_array($result_season)) {
            foreach ($result_season as $season){
                if($season['no']==$sea_no){$class="class='season'";

                $css = "style='background-color: #7cb2ff;'";

                }else{$class="class='season'";  $css =""; }
        ?>
                <div <?=$class?> <?=$css?>>(<?=$season['rent_season_start_date']?>~<?=$season['rent_season_end_date']?>)<br><a href="?linkpage=rent&subpage=season&rent_company=<?=$com_no?>&season_no=<?=$season['no']?>" ><?=$season['rent_season_name']?></a> <input type="button" id="season_up_btn" value="수정" onclick="season_up_pop(<?=$season['no']?>);"><input type="button" id="season_del_btn" value="삭제" onclick="season_del(<?=$season['no']?>);"></div>
                <?
            }
        }else{
    ...
    <p style="clear:both;padding-top: 10px;padding-bottom: 10px;">
        <img src="./image/sel_mod.gif"  id="amt_up_btn" style="cursor: pointer;" /> <img src="./image/sel_del.gif"  id="amt_del_btn" style="cursor: pointer;" />
        <img src="./image/season_add.gif"  id="season_in_btn" style="cursor: pointer;" /> 
            ...
            if(is_array($result_list)) {
                foreach ($result_list as $data){
                    ...
                    <tr>
                        ...
                        <td class="con"><?=$data['rent_season_name']?><br><?=$data['rent_season_start_date']?><br><?=$data['rent_season_end_date']?></td>
                        <td class="con"><input type="text" name="rent_season_amount_6hour[]" size="6" value="<?=$rent_amount_6hour?>">원</td>
                        <td class="con"><input type="text" name="rent_season_amount_12hour[]" size="6" value="<?=$rent_amount_12hour?>">원</td>
                        <td class="con"><input type="text" name="rent_season_amount_24hour[]" size="6" value="<?=$rent_amount_24hour?>">원</td>
                        <td class="con"><input type="text" name="rent_season_sale_car[]" size="2" value="<?=$data['rent_season_sale_car']?>">%<br><input type="text" name="rent_season_sale_car_week[]" size="2" value="<?=$data['rent_season_sale_car_week']?>">%</td>
                        <td class="con"><input type="text" name="rent_season_sale_aircar[]" size="2" value="<?=$data['rent_season_sale_aircar']?>">%<br><input type="text" name="rent_season_sale_aircar_week[]" size="2" value="<?=$data['rent_season_sale_aircar_week']?>">%</td>
                        <td class="con"><input type="text" name="rent_season_sale_aircartel[]" size="2" value="<?=$data['rent_season_sale_aircartel']?>">%<br><input type="text" name="rent_season_sale_aircartel_week[]" size="2" value="<?=$data['rent_season_sale_aircartel_week']?>">%</td>
                        <td class="con"><input type="text" name="rent_season_sale_deposit[]" size="2" value="<?=$data['rent_season_sale_deposit']?>">%<br><input type="text" name="rent_season_sale_deposit_week[]" size="2" value="<?=$data['rent_season_sale_deposit_week']?>">%</td>
                        <td class="con"><input type="text" name="rent_season_sale_additional[]" size="5" value="<?=$rent_sale_additional?>">원</td>
                        <td class="con">
                            <?php
                            for($w=0;$w <= 6;$w++){
...
<div id="layer_d_season">
    <div class="layer_d_season">
        ...
        <p style="margin-top: 10px;text-align:center;"><input id="add_btn" type="button" value="기간등록"></p>
    </div>
</div>
...
<script>
    $(document).ready(function () {
        ...
        $("#rent_company").on('change',function () {
            window.location.href ="?linkpage=rent&subpage=season&rent_company="+$("select[name=rent_company]").val()+"&rent_car_type="+$("select[name=rent_car_type]").val();
        });
        $("#rent_car_type").on('change',function () {
            window.location.href ="?linkpage=rent&subpage=season&rent_company="+$("select[name=rent_company]").val()+"&rent_car_type="+$("#rent_car_type").val();
        });
        ...
        $("#rent_all_btn").click(function () {
            ...
            $.ajax({
                type: "POST",
                url: url,
                data: $("#rent_all_frm").serialize()+"&rent_com_no="+$('select[name=rent_company]').val()+"&rent_car_type="+$('select[name=rent_car_type').val()+"&season_no=<?=$sea_no?>&case=season_all_sale", // serializes the form's elements.
                success: function(data)
                {
