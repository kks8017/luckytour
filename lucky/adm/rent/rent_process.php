...
if($case == "com_insert"){
    ...
    $rent_car_sort       = $_REQUEST["rent_car_sort"];
    $rent_car_type       = $_REQUEST["rent_car_type"];
    $rent_car_fuel       = $_REQUEST["rent_car_fuel"];
    $rent_car_seater     = $_REQUEST["rent_car_seater"];
    ...
    $sql = "update rent_car_detail 
                set 
                        ...
                        rent_car_seater     = '{$rent_car_seater}',
...
}else if($case =="car_all_sale"){


    $rent_com_no = $_REQUEST['rent_com_no'];
    $rent_car_type = $_REQUEST['rent_car_type'];
    $rent_sale_car             = $_REQUEST["rent_sale_car"] ;
    $rent_sale_aircar          = $_REQUEST["rent_sale_aircar"];
    $rent_sale_aircartel       = $_REQUEST["rent_sale_aircartel"] ;
    $rent_sale_car_week        = $_REQUEST["rent_sale_car_week"] ;
    $rent_sale_aircar_week     = $_REQUEST["rent_sale_aircar_week"] ;
    $rent_sale_aircartel_week  = $_REQUEST["rent_sale_aircartel_week"] ;
    $rent_sale_deposit         = $_REQUEST["rent_sale_deposit"] ;
    $rent_sale_deposit_week    = $_REQUEST["rent_sale_deposit_week"] ;
    if($rent_sale_car!="") {
        $sql_car[] = "rent_sale_car      ='{$rent_sale_car}'";
    }
    if($rent_sale_aircar!="") {
        $sql_car[] = "rent_sale_aircar      ='{$rent_sale_aircar}'";
    }
    if($rent_sale_aircartel!=""){
        $sql_car[] = "rent_sale_aircartel      ='{$rent_sale_aircartel}'";
    }
    if($rent_sale_car_week!=""){
        $sql_car[] = "rent_sale_car_week      ='{$rent_sale_car_week}'";
    }
    if($rent_sale_aircar_week!=""){
        $sql_car[] = "rent_sale_aircar_week      ='{$rent_sale_aircar_week}'";
    }
    if($rent_sale_aircartel_week!=""){
        $sql_car[] = "rent_sale_aircartel_week      ='{$rent_sale_aircartel_week}'";
    }
    if($rent_sale_deposit!=""){
        $sql_car[] = "rent_sale_deposit      ='{$rent_sale_deposit}'";
    }
    if($rent_sale_deposit_week!=""){
        $sql_car[] = "rent_sale_deposit_week      ='{$rent_sale_deposit_week}'";
    }
    $sql_s = implode(",",$sql_car);

    if($rent_car_type!=""){
        $type_sql = "and rent_car_type='{$rent_car_type}'";
    }

    $main_com_no = get_rentcar_company($rent_com_no,"");
    ...
    $sql_sale = "select rent_amount.no from rent_amount,rent_car_detail  where  rent_amount.rent_com_no=rent_car_detail.rent_com_no {$company_sql}  and rent_amount.rent_com_no='{$rent_com_no}' {$type_sql}  group by  rent_amount.no";

    $rs_sale  = $db->sql_query($sql_sale);
    while ($row_sale = $db->fetch_array($rs_sale)) {
      $sql = "update rent_amount 
                set 
                     {$sql_s}
                 where no='{$row_sale['no']}'";
        echo $sql;
        $db->sql_query($sql);
    }
    ...
    $sql_com = "select * from rent_amount where rent_com_no='{$rent_com_no}'";
    ...
    while ($row_com = $db->fetch_array($rs_com)) {
        ...
        $sql = "insert into rent_season_amount(rent_season_no,
                    ...
                    VALUES('$season_id',
                            ...
                            '{$row_com['rent_sale_additional']}',
                            '{$row_com['rent_sale_car']}',
                            '{$row_com['rent_sale_aircar']}',
                            '{$row_com['rent_sale_aircartel']}', 
                            '{$row_com['rent_sale_car_week']}', 
                            '{$row_com['rent_sale_aircar_week']}', 
                            '{$row_com['rent_sale_aircartel_week']}', 
                            '{$row_com['rent_sale_deposit']}', 
                            '{$row_com['rent_sale_deposit_week']}', 
                            '{$row_com['rent_sale_week']}', 
                            '{$indate}'
                            ) ";
        echo $sql;
        $db->sql_query($sql);
...
}else if($case =="season_all_sale"){


    $rent_com_no = $_REQUEST['rent_com_no'];
    $rent_car_type = $_REQUEST['rent_car_type'];
    $rent_season_no = $_REQUEST['season_no'];
    $rent_sale_car             = $_REQUEST["rent_sale_car"] ;
    $rent_sale_aircar          = $_REQUEST["rent_sale_aircar"];
    $rent_sale_aircartel       = $_REQUEST["rent_sale_aircartel"] ;
    $rent_sale_car_week        = $_REQUEST["rent_sale_car_week"] ;
    ...
    $rent_sale_deposit_week    = $_REQUEST["rent_sale_deposit_week"] ;
    if($rent_sale_car!="") {
        $sql_car[] = "rent_season_sale_car      ='{$rent_sale_car}'";
    }
    if($rent_sale_aircar!="") {
        $sql_car[] = "rent_season_sale_aircar      ='{$rent_sale_aircar}'";
    }
    if($rent_sale_aircartel!=""){
        $sql_car[] = "rent_season_sale_aircartel      ='{$rent_sale_aircartel}'";
    }
    if($rent_sale_car_week!=""){
        $sql_car[] = "rent_season_sale_car_week      ='{$rent_sale_car_week}'";
    }
    if($rent_sale_aircar_week!=""){
        $sql_car[] = "rent_season_sale_aircar_week      ='{$rent_sale_aircar_week}'";
    }
    if($rent_sale_aircartel_week!=""){
        $sql_car[] = "rent_season_sale_aircartel_week      ='{$rent_sale_aircartel_week}'";
    }
    if($rent_sale_deposit!=""){
        $sql_car[] = "rent_season_sale_deposit      ='{$rent_sale_deposit}'";
    }
    if($rent_sale_deposit_week!=""){
        $sql_car[] = "rent_season_sale_deposit_week      ='{$rent_sale_deposit_week}'";
    }
    $sql_s = implode(",",$sql_car);
    ...
    if($rent_season_no!=""){
        $season_sql = "and rent_season_no='{$rent_season_no}'";
    }
    ...
    $sql_sale = "select rent_season_amount.no from rent_season_amount,rent_car_detail  where  rent_season_amount.rent_com_no=rent_car_detail.rent_com_no {$company_sql}  and rent_season_amount.rent_com_no='{$rent_com_no}' {$type_sql} {$season_sql}  group by  rent_season_amount.no";
    echo $sql_sale;
    $rs_sale  = $db->sql_query($sql_sale);
    while ($row_sale = $db->fetch_array($rs_sale)) {
        $sql = "update rent_season_amount 
