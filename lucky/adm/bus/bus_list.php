    ...
    function car_pop(no) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $(".inbody").html(data); // show response from the php script.
                  console.log(data);
                ...
                if(is_array($result_list)) {
                    foreach ($result_list as $data){
                        ...
                        <tr>
                            ...
                            <td class="con"><input type="text" name="bus_sort_no[]" value="<?=$data['bus_sort_no']?>" size="3"></td>
                            <td class="con"><input type="checkbox" name="bus_open[]" value="Y" <?if($data['bus_open']=="Y"){?>checked<?}?>></td>
                            <td class="con"><img src="<?=$image_dir."/thumbnail_".$data['bus_image']?>" class="photo"><input type="hidden" name="old_image[]" id="old_image" value="<?=$data['bus_image']?>"></td>
                            <td class="con"><a href="javascript:;" onclick="car_pop(<?=$data['no']?>);" ><?=$data['bus_name']?></a></td>
