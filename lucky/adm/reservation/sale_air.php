...
$sql = "select * from reservation_air where reserv_user_no='{$reserv_user_no}' and  reserv_air_type='S' and reserv_del_mark!='Y'  order by no ";
...
?>
    <script>
        function air_sum(i) {
            var sum1 = get_comma($("#reserv_air_deposit_price_"+i).val());
            var sum2 = $("#reserv_air_total_price_"+i).val();
            var total = Number(sum2) - Number(sum1);

            $("#reserv_air_balance_price_"+i).val(set_comma(total))
        }
    </script>
    <script>
        $( function() {
...
if(is_array($result_list)) {
    foreach ($result_list as $data){
        ...
        <header><p>할인항공정보 <img type="button" style="cursor: pointer;" src="../image/discount_air_btn.png" onclick="air_update('<?=$data['no']?>')" /> <img type="button" style="cursor: pointer;" src="../image/upd_btn.png" onclick="air_ledger_update('<?=$i?>')"> <img type="button" style="cursor: pointer;" src="../image/res_incom.gif" onclick="air_incom()"></p>
        </header>
        ...
        <table style="width: 100%">
            ...
            <tr>
                <th >발권정보</th>
                <td  colspan="3"><span>발권일 : <input type="text" name="reserv_air_booking_date[]"   size="15" value="<?=$data['reserv_air_booking_date']?>" class="a_date_<?=$i?>">  &nbsp; 예약번호 : <input type="text" name="reserv_air_booking_number[]" id="stxt" size="15" value="<?=$data['reserv_air_booking_number']?>"> <img type="button" style="cursor: pointer;vertical-align: middle;" src="../image/air_sms.gif" onclick="air_sms_send('<?=$data['reserv_user_no']?>','<?=$data['no']?>');" > &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;블럭<input type="checkbox" name="air_block_<?=$i?>" value="Y" <?if($data['reserv_air_block']=="Y"){?>checked<?}?>> 대기<input type="checkbox" name="air_wait_<?=$i?>" value="Y" <?if($data['reserv_air_wait']=="Y"){?>checked<?}?>></span></td>
            </tr>
            <tr>
                <th >성인탑승명단</th>
            ...
            <tr>
                ...
                <th >인원</th>
                <td >성인 : <?=$data['reserv_air_adult_number']?>명, 소아 : <?=$data['reserv_air_child_number']?>명, 유아 : <?=$data['reserv_air_baby_number']?>명</td>
            </tr>
            <tr>
            ...
            <input type="hidden" id="reserv_air_total_price_<?=$i?>" value="<?=$data['reserv_air_total_deposit_price']?>">
            <?php
...
if(is_array($result_list)) {
    ...
    <div>
        <table style="width: 100%;height:50px;text-align: center;font-weight:bold;margin-top: 5px;margin-bottom: 5px;border: 1px solid #aaa;">
            <tr>
                <th ><span>비고</span></th>
                <td > <textarea name="reserv_air_bigo" id="reserv_air_bigo" rows="4" cols="73"><?=$bigo['reserv_air_bigo']?></textarea></td>
            </tr>
        </table>
    </div>
<?}?>