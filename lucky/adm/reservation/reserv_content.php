...
$sql_content = "select * from reservation_content where reserv_user_no='{$no}' order by indate desc";
...
<div id="wrapcon">
    <div >
        <table>
            <tr>
                <td class="title">변경자</td>
                <td class="title">제목</td>
                <td class="title">변경일시</td>
                <td class="title">아이피</td>
            </tr>
            <?php
            $i=0;
            if(is_array($result)) {
            ...
            ?>
            <tr>
                <td class="cont"><?=$data['person']?></td>
                <td class="cont"><a href="javascript:content(<?=$i?>);"><?=$data['reserv_title']?></a></td>
                <td class="cont"><?=$data['indate']?></td>
                <td class="cont"><?=$data['ip']?></td>
            </tr>
                <tr id="content_<?=$i?>" style="display:none;">
                    <td colspan="4" class="detail"><?=$data['reserv_content']?></td>
                </tr>
                <?php
                $i++;
            }
