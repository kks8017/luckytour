...
<table >
    <tr >
        ...
        <td ><input type="text" name="lodging_name" value="<?=$row['lodging_name']?>"></td>
        <td class="title">계좌번호</td>
        <td><input type="text" name="lodging_account" value="<?=$row['lodging_account']?>"></td>
    </tr>
    <tr>
        <td >숙소전화번호</td>
        <td ><input type="text" name="lodging_real_phone" value="<?=$row['lodging_real_phone']?>"></td>
        <td >예약실전화번호</td>
        <td><input type="text" name="lodging_manager_phone" value="<?=$row['lodging_manager_phone']?>"></td>
    </tr>
    <tr>
        <td >숙소팩스</td>
        <td><input type="text" name="lodging_real_fax" value="<?=$row['lodging_real_fax']?>"></td>
        <td >예약실팩스</td>
        <td><input type="text" name="lodging_manager_fax" value="<?=$row['lodging_manager_fax']?>"></td>
    </tr>
    <tr>
        <td >숙소이메일</td>
        <td><input type="text" name="lodging_real_email" value="<?=$row['lodging_real_email']?>"></td>
        <td >예약실이메일</td>
        <td><input type="text" name="lodging_manager_email" value="<?=$row['lodging_manager_email']?>"></td>
    </tr>
    <tr>
        <td >주소</td>
        <td colspan="3"><input type="text" name="lodging_address" value="<?=$row['lodging_address']?>"></td>
    </tr>
    <tr>
        <td>숙소위치</td>
        <td colspan="3">
            <?foreach ($lodging_list_area as $area){
                ...
                echo " <input type='radio' name='lodging_area' value='{$area['no']}' {$sel}>{$area['lodging_config_name']}";
            }
            ?>
        </td>
    </tr>
    <tr>
        <td >숙소지도설정</td>
        <td colspan="3">
            timestamp : <input type="text" name="lodging_timestamp" value="<?=$row['lodging_timestamp']?>"> key : <input type="text" name="lodging_key" value="<?=$row['lodging_key']?>">
        </td>
    </tr>
    <tr>
        ...
        <td colspan="3">
            <?foreach ($lodging_list_type as $type){
                ...
                echo "<input type='radio' name='lodging_type' value='{$type['no']}' {$sel}>{$type['lodging_config_name']}";
            }
            ?>
          </td>
    </tr>
    <tr>
        <td class="title">숙소테마</td>
        <td colspan="3">
            <?foreach ($lodging_list_thema as $thema){
                if($row['lodging_type']==$thema['no']){$sel="checked";}else{$sel="";}
                echo "<input type='checkbox' name='lodging_thema[]' value='{$thema['no']}' {$sel}>{$thema['lodging_config_name']}";
            }
            ?>
        </td>
    </tr>
    <tr>
        <td class="title">1인요금</td>
        <td colspan="3">
            <textarea  id="lodging_additional" cols="112" rows="5"><?=$row['lodging_additional']?></textarea>
        </td>
    </tr>
    <tr>
        <td class="title">숙소기본설명</td>
        <td colspan="3">
            <textarea  id="lodging_content" cols="112" rows="20"><?=$row['lodging_content']?></textarea>
        </td>
    </tr>
    <tr>
        <td class="title">숙소부대시설</td>
        <td colspan="3"><textarea  id="lodging_facility" cols="112" rows="5"><?=$row['lodging_facility']?></textarea></td>
    </tr>
    <tr>
        <td class="title">숙소이미지<br><a href="javascript:" class="up_btn">이미지등록</a></td>
        <td colspan="3">
...
<p><input id="up_btn" type="button" value="숙소수정"></p>
            ...
            $("#up_btn").click(function () {
                ...
                var facility = oEditors3.getById["lodging_facility"].getIR();	// 에디터의 내용이 textarea에 적용됩니다.
                    $.ajax({
                        type: "POST",
                        url: url,
                        data: $("#tel_up_frm").serialize()+"&lodging_additional="+additional+"&lodging_content="+content+"&lodging_facility="+facility, // serializes the form's elements.
                        success: function (data) {
