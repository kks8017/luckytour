    ...
    <div id="tel_amount">
        ...
        <form id="margin_frm">
            <table>
                ...
                <tr>
                    <td><input type="text" name="margin_basic" value="<?=$row_margin['golf_amount_margin_basic']?>"></td>
                    <td><input type="text" name="margin_amount" value="<?=$row_margin['golf_amount_margin_amount']?>"></td>
        ...
        <div>기간리스트</div>
        <div class="season">
            <div <?if(!$season_no){?>class="div_bule"<?}?>><a href="?linkpage=<?=$linkpage?>&subpage=amount&golf_no=<?=$golf_no?>">비수기</a></div>
            <?php
            if(is_array($result_season)) {
                foreach ($result_season as $season){
                    if($season['no']==$season_no){$class = "class='div_bule'";}else{$class="";}
                    ?>
                    <div <?=$class?>><a href="?linkpage=<?=$linkpage?>&subpage=amount&golf_no=<?=$golf_no?>&mode=season&season_no=<?=$season['no']?>"><?=$season['golf_season_name']?>(<?=$season['golf_season_start_date']?>~<?=$season['golf_season_end_date']?>)</a> <input type="button" id="season_up_btn" value="수정" onclick="season_up_pop(<?=$season['no']?>);"><input type="button" id="season_del_btn" value="삭제" onclick="season_del(<?=$season['no']?>);"></div>
                    <?
                }
            }else{
                echo "";
                ...
                if(is_array($result_list)) {
                    foreach ($result_list as $data){
                        ...
                        <tr>
                            ...
                            <?if($mode=="season"){?>
                                <td><input type="checkbox"  name="golf_season_open[]" value="Y" <?=$open_chk?>></td>
                            <?}?>
                            <td><input type="text" name="golf_amount_basic[]" value="<?=$golf_amount_basic?>" size="8"><br><input type="text" name="golf_amount_basic_week[]" value="<?=$golf_amount_basic_week?>" size="8"></td>
                            <td><input type="text" name="golf_amount_amount[]" value="<?=$golf_amount_amount?>" size="8"><br><input type="text" name="golf_amount_amount_week[]" value="<?=$golf_amount_amount_week?>" size="8"></td>
                            <td><input type="text" name="golf_amount_deposit[]" value="<?=$golf_amount_deposit?>" size="8"><br><input type="text" name="golf_amount_deposit_week[]" value="<?=$golf_amount_deposit_week?>" size="8"></td>
