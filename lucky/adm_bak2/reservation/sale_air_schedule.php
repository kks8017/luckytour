...
$stay = $ddy."박".($ddy+1)."일";
$sql = "select no,a_sch_departure_area_name,
                ...
                a_sch_bigo,
                a_sch_company
        from air_schedule  where a_sch_departure_date = '{$start_date}' and a_sch_departure_area_name='{$area}' and a_sch_stay='{$stay}'  $sql_line $sql_company $sql_time $sql_sale  order by a_sch_departure_date,a_sch_departure_time,a_sch_arrival_date,a_sch_arrival_time";
//echo $sql;
$rs  = $db->sql_query($sql);
    ...
    <div>
        <table>
            <tr>
                ...
                <td>성인판매요금<br>소아판매요금</td>
                <td>성인입금가<br>소아입금가</td>
                <td>항공사<br>판매사</td>
                <td>비고</td>
                <td>추가</td>
            </tr>
            ...
            <tr>
                ...
                <td>성인:<?=$adult_number?><br>소아:<?=$child_number?><br>유아:<?=$baby_number?></td>
                <td><?=$data['a_sch_adult_sale']?><br><?=$data['a_sch_child_sale']?></td>
                <td><?=set_comma($a_sch_adult_normal_price)?><br><?=set_comma($a_sch_child_normal_price)?></td>
                ...
                <td><?if($type=="add"){?><input type="button"   value="추가" onclick="air_add('<?=$data['no']?>','<?=$adult_number?>','<?=$child_number?>','<?=$baby_number?>');"><?}else{?><input type="button" value="변경" onclick="air_update('<?=$data['no']?>','<?=$adult_number?>','<?=$child_number?>','<?=$baby_number?>');"><?}?></td>
            </tr>
                <?php
                $i++;
            }
                ...
                ?>
                <tr>
                    <th colspan="11" class="tb_center"><p>등록된 정보가 없습니다.</p></th>
                </tr>
            <?}?>
        </table>
    </div>
