...
if($case=="insert"){
    ...
    $sql_sch = "select * from air_schedule where no='{$air_no}'";
   // echo $sql_sch;
    ...
    $reserv_air_normal_total_price = (($a_sch_adult_normal_price * $reserv_air_adult_number) + ($a_sch_child_normal_price * $reserv_air_child_number));
    $total_price = (($row_sch['a_sch_adult_sale_price'] * $reserv_air_adult_number) + ($row_sch['a_sch_child_sale_price'] * $reserv_air_child_number));
    $total_deposit_price = (($row_sch['a_sch_adult_deposit_price']* $reserv_air_adult_number) + ($row_sch['a_sch_child_deposit_price'] * $reserv_air_child_number));
  //  echo "$total_deposit_price = (({$row_sch['a_sch_adult_deposit_price']}* {$reserv_air_adult_number}) + ({$row_sch['a_sch_child_deposit_price']} + {$reserv_air_child_number}))";
   ...
   // echo $sql;
    ...
    $reserv_air_departure_date = $start_year."-".$start_month."-".$start_day." ".$start_hour.":".$start_minute;
    $reserv_air_arrival_date   = $end_year."-".$end_month."-".$end_day." ".$end_hour.":".$end_minute;

    $ddy = ( strtotime($end_year."-".$end_month."-".$end_day) - strtotime($start_year."-".$start_month."-".$start_day) ) / 86400;
    $stay = $ddy."박".($ddy+1)."일";
    ...
    $reserv_air_deposit_state  = $_REQUEST['reserv_air_deposit_state'][$i];
    ...
    $reserv_air_balance_state     = $_REQUEST['reserv_air_balance_state'][$i];
