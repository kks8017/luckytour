...
$sql = "select * from reservation_air where reserv_user_no='{$reserv_user_no}' and  reserv_air_type='S' and reserv_del_mark!='Y'  order by no desc";
$rs  = $db->sql_query($sql);
...
?>
    ...
    <table>
        <tr>
            <td colspan="4">할인항공정보 <input type="button"  value="할인항공변경" onclick="air_update('<?=$data['no']?>')"> <input type="button" value="수정" onclick="air_ledger_update('<?=$i?>')"></td>
        </tr>
        <tr>
            <td class="title"><?=$data['reserv_air_departure_area']?>->제주 <input type="hidden" name="reserv_air_no[]" value="<?=$data['no']?>"></td>
    ...
    <table class="total">
        <tr>
            ...
            <td>수  익 : <?=$profit?>원</td>
            <td>정요금 : <?=$data['reserv_air_normal_total_price']?>원</td>
        </tr>
    </table>
 <?php
    $i++;
...
<div>
    <table>
        <tr>
            <td >비고</td>
            <td > <textarea name="reserv_air_bigo" id="reserv_air_bigo" rows="4" cols="80"></textarea></td>
        </tr>
    </table>
</div>
<?}?>