...
if(is_array($_POST['adult_name'])){
    $adult_name_list = implode(",",$_POST['adult_name']);
}
if(is_array($_POST['adult_name'])){

    for ($i=0;$i < count($_POST['child_name']);$i++ ){
        $child_name[]=  $_POST['child_name'][$i]."(".$_POST['child_brith'][$i].")";
    }
    $child_name_list = implode(",",$child_name);
}
if(is_array($_POST['baby_name'])){

    for ($i=0;$i < count($_POST['baby_name']);$i++ ){
        $baby_name[]=  $_POST['baby_name'][$i]."(".$_POST['baby_brith'][$i].")";
    }
    $baby_name_list = implode(",",$baby_name);
}
$ip = $main->user_ip();
$incom = "MOBILE";
...
$reserv_date  = date("Y-m-d",time());
$balance_price = $total_price - $reserv_price;
$reserv_date  = date("Y-m-d", strtotime(time()." +7 days"));
...
$sql_amount = "insert into reservation_amount(reserv_user_no,reserv_deposit_price,reserv_deposit_date,reserv_balance_price,reserv_balance_date,reserv_total_price,indate) 
                                        values('{$reserv_no}','{$reserv_price}','{$reserv_date}','{$balance_price}','{$reserv_date}','{$total_price}','{$indate}')";
...
//echo $sql_amount;
echo "<script>
            window.location.href = 'res_end.php?reserv_no={$reserv_no}'
       </script>
      ";
?>