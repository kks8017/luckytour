		...
		<tr>
			...
			<td class="select-text">
				<select name="time">
					...
					<option value="1">06:00~07:59</option>
					<option value="2">08:00~09:59</option>
...
<div class="button-summit-area">
	<button class="button-summit" id="search_btn" type="summit">패키지 검색</button>
</div>

<div id="air_schedule" class="select-air-list">
...
<script type="text/javascript">
    function schedule(pagenum,re) {
        ...
        $.ajax({
            type: "POST",
            url: url,
            data: "start_date="+$("#start_date").val()+"&end_date="+$("#end_date").val()+"&stay="+$("select[name=stay]").val()+"&adult_number="+$("#adult_number").val()+"&child_number="+$("#child_number").val()+"&baby_number="+$("#baby_number").val()+"&package="+$("select[name=package]").val()+"&area="+$("select[name=start_area]").val()+"&time="+$("select[name=time]").val()+"&package_type=<?=$package_type?>&pagenum="+pagenum2, // serializes the form's elements.
