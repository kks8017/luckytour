...
class lodging {
    ...
    public function lodging_sub_image(){
        $sql = "select lodging_image_file_m from lodging_image where lodging_no='{$this->lodno}' order by no asc limit 5";
        $rs  = $this->db->sql_query($sql);
    ...
    public function room_sub_image($img_number=""){

        $sql = "select lodging_room_image_file_m from lodging_room_image where lodging_room_no='{$this->roomno}'   order by no asc limit {$img_number}";
     //   echo $sql;
        $rs  = $this->db->sql_query($sql);
        while ($row = $this->db->fetch_array($rs)){
            $result[] = $row;
    ...
    public function lodging_add_price($adult_add="",$child_add=""){
       ...
       // echo $sql;
        ...
        $add_adult_number = $this->adult_number - $row['lodging_room_min'];
        ...
        if($this->adult_number == $row['lodging_room_min']){
            ...
            if($this->adult_number < $row['lodging_room_min']) {
                ...
                if($tmp_child_number >0) {
                    ...
                    $add_child_number = $total_number - $row['lodging_room_min'];
                    $add_child_number = $add_child_number - $add_adult_number;
                }else{
                    $add_child_number = 0;
                }
            }
    ...
    function lodging_season_price($lodging_season_no){
      ...
      //  echo $sql."<br>";
        ...
        if(strpos($row['lodging_season_week'],$start_week)!==false) {
            $basic_price = $row['lodging_season_basic_week'];
            ...
            $basic_price = $row['lodging_season_basic'];
    ...
    public function lodging_main_price(){
        $tel_price = $airtel_price = $aircartel_price = $add_price =  $deposit_price = $basic_price = 0;
        $lodging_season_no = $this->lodging_season_list();
        for($i=0;$i < $this->stay ;$i++) {
            ...
            if ($lodging_season_no) {
               $lodging_price =  $this->lodging_season_price($lodging_season_no);
                $tel_price += $lodging_price[0]+ $lodging_price[3];
                $airtel_price += $lodging_price[1] + $lodging_price[3];
                $aircartel_price += $lodging_price[2] + $lodging_price[3];
                $add_price += $lodging_price[3];
                $deposit_price += $lodging_price[4] + $lodging_price[3];
                $basic_price += $lodging_price[5] + $lodging_price[3];
              } else {
