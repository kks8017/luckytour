...
class report{
    ...
    public $all_price,$refund,$air_block,$rent_block,$lod_block,$etc_type,$etc_date,$etc_state;
    ...
    function air_report(){
        ...
        $sql = "select *,reservation_air.no as air_no from reservation_air,reservation_user_content  where reservation_user_content.no=reservation_air.reserv_user_no and  reservation_air.reserv_del_mark!='Y' and reserv_state='OK' {$sch_sql} order by reserv_air_departure_date asc,reserv_air_arrival_date asc";
    ...
    function reservation_collect_etc_report(){
        $sch_sql = "";
        if($this->start_date){
            $sch_sql .= "and  {$this->search_date} between '{$this->start_date}' and '{$this->end_date}' ";
        }
        if($this->etc_type!=""){
            $sch_sql .= "and reserv_etc_type='{$this->etc_type}'";
        }
        if($this->etc_state!=""){
            $sch_sql .= "and reserv_etc_state='{$this->etc_state}'";
        }
        if($this->staff){
            $sch_sql .= "and reserv_person='{$this->staff}'";
        }
        $sql = "select * from reservation_amount_etc,reservation_amount,reservation_user_content where reservation_amount_etc.reserv_amount_no=reservation_amount.no and  reservation_user_content.no=reservation_amount.reserv_user_no   {$sch_sql} order by reserv_etc_date asc";
        $rs  = $this->db->sql_query($sql);
        while ($row = $this->db->fetch_array($rs)){
            $result[] = $row;
        }
    ...
    function reservation_collect_card_report(){
        ...
        $sql = "select * from reservation_amount,reservation_user_content,reservation_amount_card where reservation_user_content.no=reservation_amount.reserv_user_no and reservation_amount.no=reservation_amount_card.reserv_amount_no and reserv_del_mark!='Y'   {$sch_sql} order by {$this->search_date} asc";
    ...
    function reservation_sell_state(){
        ...
        switch ($row["{$this->reserv_price_state}"]){
            case "Y" :
                $state = "<span style='color: red'>입금완료</span>";
                break;
            default :
                $state = "<span style='color: blue'>입금요청</span>";

        }

        return $state;
    }
    function reservation_state($state){
    ...
    $sql = "select cnt_wt,cnt_bl,cnt_ok,cnt_cl,cnt_gl,cnt_bj,cnt_bw,total_sum from 
                ...
                (select count(no) as cnt_bw from reservation_user_content where reserv_del_mark!='Y' and reserv_state='BW' {$sch_sql}) as j,
    ...
    function reservation_state_total(){
        $sch_sql = "";
        if($this->start_date){
            $sch_sql .= "and reservation_user_content.{$this->search_date} between '{$this->start_date}' and '{$this->end_date}' ";
        }
        ...
        $sql = "select cnt_wt,cnt_bl,cnt_ok,cnt_cl,cnt_gl,cnt_bj,total_sum from 
                ...
                (select count(no) as cnt_gl from reservation_user_content where reserv_del_mark!='Y' and reserv_state='GL' {$sch_sql}) as e,
                (select count(no) as cnt_bj from reservation_user_content where reserv_del_mark!='Y' and reserv_state='BJ' {$sch_sql}) as f,
                (select sum(reserv_total_price) as total_sum from reservation_amount,reservation_user_content where reservation_user_content.no=reservation_amount.reserv_user_no and reserv_del_mark!='Y' and reserv_state='OK'  {$sch_sql}) as g
    ...
    function reserv_deposit_total_price(){
        $sch_sql = "";
        if($this->start_date){
            $sch_sql .= "and reservation_user_content.{$this->search_date} between '{$this->start_date}' and '{$this->end_date}' ";
        }
        ...
        $sql = "select  air_price ,rent_price ,lod_price ,bus_price ,bustour_price ,golf_price,equip_price 
                  from  (select sum(reserv_air_total_deposit_price) as air_price from reservation_air,reservation_user_content where reservation_user_content.no=reservation_air.reserv_user_no and   reservation_air.reserv_del_mark!='Y'and reserv_state='OK' {$sch_sql}) as a,
                         (select sum(reserv_rent_total_deposit_price) as rent_price from reservation_rent,reservation_user_content where reservation_user_content.no=reservation_rent.reserv_user_no and  reservation_rent.reserv_del_mark!='Y' and reserv_state='OK' {$sch_sql}) as b,
                         (select sum(reserv_tel_total_dposit_price) as lod_price from reservation_lodging,reservation_user_content where reservation_user_content.no=reservation_lodging.reserv_user_no and   reservation_lodging.reserv_del_mark!='Y' and reserv_state='OK' {$sch_sql}) as c,
                         (select sum(reserv_bus_total_deposit_price) as bus_price from reservation_bus,reservation_user_content where reservation_user_content.no=reservation_bus.reserv_user_no  and reservation_bus.reserv_del_mark!='Y' and reserv_state='OK' {$sch_sql}) as d,
                         (select sum(reserv_bustour_total_deposit_price) as bustour_price from reservation_bustour,reservation_user_content where reservation_user_content.no=reservation_bustour.reserv_user_no and  reservation_bustour.reserv_del_mark!='Y' and reserv_state='OK' {$sch_sql}) as f,
                         (select sum(reserv_golf_total_dposit_price) as golf_price from reservation_golf,reservation_user_content where reservation_user_content.no=reservation_golf.reserv_user_no and reservation_golf.reserv_del_mark!='Y' and reserv_state='OK' {$sch_sql}) as g,
                         (select sum(reserv_equip_total_deposit_price) as equip_price from reservation_equip,reservation_user_content where reservation_user_content.no=reservation_equip.reserv_user_no and reservation_equip.reserv_del_mark!='Y' and reserv_state='OK' {$sch_sql}) as h
                  ";
...
//        echo (int)$row['air_price']."=".(int)$row['rent_price']."=".(int)$row['lod_price']."=".(int)$row['bus_price']."=".(int)$row['bustour_price']."=".(int) $row['golf_price'];

        $total_price = (int)$row['air_price'] + (int)$row['rent_price'] + (int)$row['lod_price'] + (int)$row['bus_price'] + (int)$row['bustour_price'] +(int) $row['golf_price']+(int) $row['equip_price'];
    ...
    function reservation_profit(){
        $total_amount = $this->reservation_state_total();
        $total_profit = $this->reserv_deposit_total_price();
    ...
    function air_block_list(){
        ...
        if($this->air_block=="block"){
            $sch_sql .= "and reservation_air.reserv_air_block='Y'";
        }else if($this->air_block=="wait"){
            $sch_sql .= "and reservation_air.reserv_air_wait='Y'";
        }else{
            $sch_sql .= "and (reservation_air.reserv_air_block='Y' or reservation_air.reserv_air_wait='Y')";
        }
        $sql = "select * from reservation_air,reservation_user_content  where reservation_user_content.no=reservation_air.reserv_user_no and  reservation_air.reserv_del_mark!='Y'  and reserv_air_block='Y'  {$sch_sql} order by reserv_air_departure_date asc,reserv_air_arrival_date asc";
        $rs  = $this->db->sql_query($sql);
        while ($row = $this->db->fetch_array($rs)){
            $result[] = $row;
        }
    ...
    function rent_block_list(){
        ...
        if($this->rent_block=="block"){
            $sch_sql .= "and reservation_rent.reserv_rent_block='Y'";
        }else if($this->rent_block=="wait"){
            $sch_sql .= "and reservation_rent.reserv_rent_wait='Y'";
        }else{
            $sch_sql .= "and (reservation_rent.reserv_rent_wait='Y' or reservation_rent.reserv_rent_block='Y')";
        }
        $sql = "select * from reservation_rent,reservation_user_content where reservation_user_content.no=reservation_rent.reserv_user_no and  reservation_rent.reserv_del_mark!='Y'{$sch_sql} order by reserv_rent_start_date asc";
        $rs  = $this->db->sql_query($sql);
        while ($row = $this->db->fetch_array($rs)){
            $result[] = $row;
    ...
    function lodging_block_list(){
        ...
        if($this->lod_block=="block"){
            $sch_sql .= "and reservation_lodging.reserv_tel_block='Y'";
        }else if($this->lod_block=="wait"){
            $sch_sql .= "and reservation_lodging.reserv_tel_wait='Y'";
        }else{
            $sch_sql .= "and (reservation_lodging.reserv_tel_block='Y' or reservation_lodging.reserv_tel_wait='Y')";
        }
        $sql = "select * from reservation_lodging,reservation_user_content where reservation_user_content.no=reservation_lodging.reserv_user_no and  reservation_lodging.reserv_del_mark!='Y' {$sch_sql} order by reserv_tel_date asc";
        // echo $sql;
        $rs  = $this->db->sql_query($sql);
        while ($row = $this->db->fetch_array($rs)){
            $result[] = $row;
