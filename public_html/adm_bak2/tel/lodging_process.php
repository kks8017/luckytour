...
if($case =="tel_insert"){
    ...
    for($i=0; $i < count($_REQUEST["sel"]);$i++) {
        ...
        $lodging_open1    = $_REQUEST["lodging_open"][$num];
        $lodging_recomm1 = $_REQUEST["lodging_recomm"][$num];
    ...
    $lodging_theme = $_REQUEST["lodging_theme"];
  ...
  //  echo $sql_amount;
    ...
    if ($mode == "") {
        ...
        if($row_margin['cnt'] > 0){
            $sql_margin_up = "update lodging_amount_margin set lodging_amount_margin_basic='{$margin_basic}',lodging_amount_margin_tel='{$margin_tel}',lodging_amount_margin_airtel='{$margin_airtel}',lodging_amount_margin_aircartel='{$margin_aircartel}' where no='{$no}'";
            echo $sql_margin_up;
            $db->sql_query($sql_margin_up);
        }else{
            $sql_margin_in = "insert into lodging_amount_margin (lodging_no,lodging_amount_margin_basic,lodging_amount_margin_tel,lodging_amount_margin_airtel,lodging_amount_margin_aircartel) values('{$lodging_no}','{$margin_basic}','{$margin_tel}','{$margin_airtel}','{$margin_aircartel}')";
       ...
       // print_r($result_amount);
        ...
        if($row_margin['cnt'] > 0){
            ...
            $sql_margin_in = "insert into lodging_amount_margin (lodging_no,lodging_season_no,lodging_margin_mode,lodging_amount_margin_basic,lodging_amount_margin_tel,lodging_amount_margin_airtel,lodging_amount_margin_aircartel) values('{$lodging_no}','{$season_no}','season','{$margin_basic}','{$margin_tel}','{$margin_airtel}','{$margin_aircartel}')";
            $db->sql_query($sql_margin_in);
