...
//      var maskWidth = $(document).width();
    ...
    function air_update(no,num_ad,num_ch,num_yu){
        var url = "air_reserv_process.php"; // the script where you handle the form input.
        $.ajax({
            type: "POST",
            url: url,
            data: "no="+no+"&adult_number="+num_ad+"&child_number="+num_ch+"&baby_number="+num_yu+"&reserv_user_no=<?=$no?>&reserv_air_no=<?=$reserv_air_no?>&case=update", // serializes the form's elements.
...
<div class="reserv_air_add">
    <div>
        ...
        <form id="air_sch_frm">
            ...
            <table>
                <tr>
                    <td align="center"><input type="button" id="air_update_btn" value="일정수정"><input type="button" id="air_delete_btn" value="일정삭제"></td>
                </tr>
            </table>
            <table>
                <tr>
                    <td class="title" colspan="4">검색</td>
                </tr>
                <tr>
